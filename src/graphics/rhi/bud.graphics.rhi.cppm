...
export namespace bud::graphics {
	...
	public:
		...
		void init(SDL_Window* window, bool enable_validation) override {
			...
			create_render_pass();
			create_graphics_pipeline();	// Must be after render pass
			create_framebuffers();
		...
		void cleanup() override {
			...
			// Oderly destroy Vulkan resources
	...
	private:
		...
		void pick_physical_device() {
			...
			// Simple selection: Pick the first Discrete GPU
		...
		void create_logical_device(bool enable_validation) {
			...
			// Enable features here (e.g., samplerAnisotropy)
	...
	private:
		...
		VkSurfaceFormatKHR choose_swap_surface_format(const std::vector<VkSurfaceFormatKHR>& available_formats) {
			for (const auto& available_format : available_formats) {
				// 优先选择 SRGB 格式 (使得颜色更准确)
				if (available_format.format == VK_FORMAT_B8G8R8A8_SRGB &&
					available_format.colorSpace == VK_COLOR_SPACE_SRGB_NONLINEAR_KHR) {
					return available_format;
				}
			}
			return available_formats[0]; // 兜底方案
		}

		VkPresentModeKHR choose_swap_present_mode(const std::vector<VkPresentModeKHR>& available_present_modes) {
			for (const auto& available_present_mode : available_present_modes) {
				// Mailbox 模式通常被称为 "三重缓冲"，延迟低且无撕裂
				if (available_present_mode == VK_PRESENT_MODE_MAILBOX_KHR) {
					return available_present_mode;
				}
			}
			// FIFO (V-Sync) 是 Vulkan 规范强制要求必须支持的
	...
	public: // 改为 public 或者在 init 中调用
		void create_swapchain(SDL_Window* window) {
			...
			create_info.imageArrayLayers = 1; // 除非你在做 VR (Stereo 3D)，否则总是 1
			create_info.imageUsage = VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT; // 我们直接渲染到它上面

			QueueFamilyIndices indices = find_queue_families(physical_device);
			uint32_t queue_family_indices[] = { indices.graphics_family.value(), indices.present_family.value() };

			if (indices.graphics_family != indices.present_family) {
				// 如果图形队列和呈现队列不同，使用并发模式避免复杂的同步所有权转移
	...
	private:
		void create_render_pass() {
			...
			color_attachment.samples = VK_SAMPLE_COUNT_1_BIT; // 不使用多重采样

			// LoadOp: 绘制前怎么处理？ -> 清除为纯色 (Clear)
			color_attachment.loadOp = VK_ATTACHMENT_LOAD_OP_CLEAR;
			// StoreOp: 绘制后怎么处理？ -> 保存起来以便显示 (Store)
			...
			// 布局转换：
			// 初始：未定义 (UNDEFINED)
			// 结束：适合展示在屏幕上 (PRESENT_SRC_KHR)
			color_attachment.initialLayout = VK_IMAGE_LAYOUT_UNDEFINED;
			color_attachment.finalLayout = VK_IMAGE_LAYOUT_PRESENT_SRC_KHR;

			VkAttachmentReference color_attachment_ref{};
			color_attachment_ref.attachment = 0; // 对应上面数组的索引
		...
		void create_framebuffers() {
			...
			std::println("[Vulkan] Framebuffers created: {}", swapchain_framebuffers.size());
		}

	private:
		static std::vector<char> read_file(const std::string& filename) {
			// 从文件末尾开始读(ate)，二进制模式(binary)
		...
		VkShaderModule create_shader_module(const std::vector<char>& code) {
			...
			// vector<char> -> uint32_t* 的转换需要注意对齐，但 vector data通常是安全对齐的
		...
		void create_graphics_pipeline() {
			...
			// 8. Pipeline Layout (管线布局，用于 uniform 等)
			...
			pipeline_info.pDepthStencilState = nullptr; // 没有深度缓冲
			pipeline_info.pColorBlendState = &color_blending;
			pipeline_info.pDynamicState = nullptr;
			pipeline_info.layout = pipeline_layout;
			pipeline_info.renderPass = render_pass; // 兼容这个 Render Pass
			...
			// 清理 Shader Module (need to wrap in RAII)
	...
	private:
		void create_command_pool() {
			...
			pool_info.flags = VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT; // 允许重置缓冲
		...
		void create_sync_objects() {
			...
			fence_info.flags = VK_FENCE_CREATE_SIGNALED_BIT; // 一开始就是“绿灯”状态，防止第一帧卡死
	...
	public:
		void draw_frame() override {
			// 1. 等待上一帧结束 (防止 CPU 把 command buffer 改写了 GPU 还在读)
			...
			// (这里简化处理，如果 Swapchain 过期应该重建，暂时忽略)
		...
		// Don't forget to wait for device idle, or it will crash on exit
	...
	private:
		// type_filter: memory type bits that are suitable
		...
		void create_vertex_buffer() {
			...
			// 创建一个既能当 Vertex Buffer 用，又能被 CPU 写入 (HOST_VISIBLE) 的内存
			// HOST_COHERENT 保证我们写入后 GPU 能立即看到，不需要手动 Flush
