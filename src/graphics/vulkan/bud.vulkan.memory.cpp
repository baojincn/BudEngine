...
namespace bud::graphics::vulkan {
    ...
    void VulkanMemoryAllocator::set_resource_pool(bud::graphics::ResourcePool* pool) {
        this->resource_pool = pool;
        if (!pool) return;
        auto* vk_pool = dynamic_cast<VulkanResourcePool*>(pool);
        for (auto& page : staging_pages) {
            if (page.buffer != VK_NULL_HANDLE && !page.pool_handle.is_valid()) {
                auto vk_buf = std::make_shared<VulkanBuffer>();
                vk_buf->buffer = page.buffer;
                vk_buf->allocation = page.allocation;
                vk_buf->alloc_info = page.alloc_info;
                vk_buf->size = page.size;
                vk_buf->mapped_ptr = page.mapped_ptr;
                vk_buf->owns_allocation = false; // staging_pages will destroy underlying VMA allocation in cleanup()
                vk_buf->allocator = vma_allocator;
                if (vk_pool) {
                    page.pool_handle = vk_pool->register_buffer(vk_buf);
                }
            }
        }
    }
        ...
        for (auto &vec : pending_buffers) {
            for (auto &handle : vec) {
                if (handle.is_valid() && resource_pool) {
                    resource_pool->release_buffer(handle);
                }
            }
            vec.clear();
        }
    ...
    bud::graphics::BufferSlice VulkanMemoryAllocator::alloc_frame_transient(uint64_t size, uint64_t alignment) {
        return alloc_staging(size, alignment);
    }
    ...
    bud::graphics::BufferSlice VulkanMemoryAllocator::alloc_staging(uint64_t size, uint64_t alignment) {
        ...
        bud::graphics::BufferSlice slice;

        if (success && current_page) {
            slice.buffer = current_page->pool_handle;
            slice.offset = offset;
            slice.size = size;
            slice.mapped_ptr = static_cast<uint8_t*>(current_page->mapped_ptr) + offset;
            return slice;
        }
        ...
        if (resource_pool) {
            BufferDesc desc;
            desc.size = size;
            desc.usage = ResourceState::Common;
            desc.memory_usage = MemoryUsage::StagingRing;
            desc.name = "StagingFallback";
            BufferHandle handle = resource_pool->acquire_buffer(desc);
            Buffer* buf = resource_pool->get_buffer(handle);
            slice.buffer = handle;
            slice.offset = 0;
            slice.size = size;
            slice.mapped_ptr = buf ? buf->mapped_ptr : nullptr;
            return slice;
        }
    ...
    bud::graphics::BufferHandle VulkanMemoryAllocator::alloc_gpu(uint64_t size, bud::graphics::ResourceState usage) {
        if (resource_pool) {
            ...
            return resource_pool->acquire_buffer(desc);
        }
        return BufferHandle{};
    }
    ...
    bud::graphics::BufferHandle VulkanMemoryAllocator::alloc_persistent(uint64_t size, bud::graphics::ResourceState usage) {
        if (resource_pool) {
            ...
            desc.size = size;
            desc.usage = usage;
            desc.memory_usage = MemoryUsage::PersistentMapped;
            return resource_pool->acquire_buffer(desc);
        }
        return BufferHandle{};
    }
    ...
    bud::graphics::BufferHandle VulkanMemoryAllocator::alloc_read_back(uint64_t size) {
        if (resource_pool) {
            BufferDesc desc;
            desc.size = size;
            desc.usage = ResourceState::TransferDst;
            desc.memory_usage = MemoryUsage::Readback;
            return resource_pool->acquire_buffer(desc);
        }
        return BufferHandle{};
    }
