...
import bud.vulkan.utils; // 引入刚才写的工具

namespace bud::graphics::vulkan {

	export class VulkanResourcePool : public ResourcePool {
	public:
	...
	private:
		...
		// 智能指针容器
		std::unordered_map<size_t, std::vector<std::unique_ptr<VulkanTexture>>> image_pool;
		...
		void destroy_vulkan_objects(VulkanTexture* tex) {
			...
			// [FIX] Do NOT destroy default_sampler here, it's shared and owned by VulkanRHI
